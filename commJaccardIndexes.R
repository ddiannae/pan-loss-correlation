...
all_interactions <- lapply(tissues, function(tissue) {
  interactions <- read_tsv(paste0(tissue, 
                                  "/network_aracne/normal-interactions-100000.tsv"))
...
all_comms <- lapply(tissues, function(tissue) {
  communities <- read_tsv(paste0(tissue, "/network_aracne/communities/normal-comm-all-100000.tsv"))
...
all_jis <- apply(tissue_combs, 1, function(row) {
  ...
  write_tsv(high_jis, file = paste0("pan-loss/community_intersections/jacc_idxs_normal_all/", 
