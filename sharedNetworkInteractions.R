...
jacc_dist <- function(set1, set2) {
  return(length(intersect(set1, set2))/length(union(set1, set2)))
}

tissue_comb <- expand_grid(t1 = tissues, t2 = tissues)

dist_cancer <- lapply(1:nrow(tissue_comb), function(i) {
    t1 <- tissue_comb[i, ] %>% pull(t1)
    t2 <- tissue_comb[i, ] %>% pull(t2)
    s1 <- cancer$all_interactions %>% filter(tissue == t1) %>% pull(inter)
    s2 <- cancer$all_interactions %>% filter(tissue == t2) %>% pull(inter) 
    return(list(t1=t1, t2=t2, jd=1-jacc_dist(s1, s2)))
  })

dist_cancer <- bind_rows(dist_cancer) %>% 
  pivot_wider(id_cols = t1, names_from = t2, values_from = jd) 
names <- dist_cancer$t1
dist_cancer <- dist_cancer %>% select(-t1) %>% as.matrix()
rownames(dist_cancer) <- names
dist_cancer <- as.dist(dist_cancer, diag = TRUE)
hclust_avg <- hclust(dist_cancer, method = "average")
plot(hclust_avg)


getIntersectionPlots <- function(cond, interactions, ts, color, ni) {
  
  rep_cancer_groups <- interactions %>% group_by(inter, interaction_type) %>% 
  ...
  csums <- colSums(rep_cancer_groups %>% select(all_of(ts)))
  
  rep_cancer_groups %>% mutate_at(all_of(ts), as.numeric) %>% 
    unite(col = "barcode", all_of(ts), sep="") %>% 
    write_tsv(file = paste0("pan-loss/", cond, "-interaction-barcodes-", ninter, ".tsv"))
...
getIntersectionPlots("cancer", cancer$all_interactions, tissues, "#a32e27", ninter)
getIntersectionPlots("normal", normal$all_interactions, tissues, "#e3a098", ninter)
getIntersectionPlots("cancer_only", cancer_only$all_interactions, tissues, "#a32e27", ninter)
getIntersectionPlots("normal_only", normal_only$all_interactions, tissues, "#e3a098", ninter)

shared$all_interactions$mi <- 1
getIntersectionPlots("shared", shared$all_interactions, tissues, "#008080", 0)
