...
tissues <- c("bladder", "brain", "breast", "esophagus", "kidney", "liver", 
             "lung", "ovary", "pancreas", "prostate", "thyroid", "uterus")

mi_data <- lapply(tissues, function(tissue) {
  bind_rows(
    vroom(paste0(tissue, "/correlation_cnv_expr/normal-intra-inter-count-log-bins.tsv")),
    vroom(paste0(tissue, "/correlation_cnv_expr/cancer-intra-inter-count-log-bins.tsv"))
...
png(paste0("pan-loss/heatmaps/cnv_expr_intra_inter.png"), width = 1200, height = 1000)
